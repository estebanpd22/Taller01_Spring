...
import unimagdalena.edu.co.Taller1.repositories.*;
...
import unimagdalena.edu.co.Taller1.services.mapper.SeatInventoryMapper;
...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public List<BookingResponse> finBookingByPassengerEmail(  String email) {
        if (email.trim().isBlank()) throw new IllegalArgumentException("BookingCreateRequest passengerEmail can't be null!");
        Page<Booking> page = bookingRepository.findByPassenger_EmailIgnoreCaseOrderByCreatedAtDesc(email, PageRequest.ofSize(10));
    ...
    @Override
    public BookingResponse getBookingWithDetails(long id) {
        return bookingRepository.fetchGraphById(id).map(BookingMapper::toResponse)
                .orElseThrow(()-> new NotFoundException("Booking Not Found"));
    }

    @Override@Transactional(readOnly = true)
    public BookingResponse getBookingId(long id) {
        Booking booking = bookingRepository.fetchGraphById(id).orElseThrow(()-> new NotFoundException("Booking Not Found"));
    ...
    private List<BookingItem> createBookinItems(List<BookingItemDtos.BookingItemCreateRequest> items, List<Flight> flights) {
        ...
        return items.stream()
                .map(item -> {
                    ...
                    return BookingItemMapper.toEntity(item, flight);
                })
                .toList();
    }
    ...
    private void validarCreateBookingRequest(BookingCreateRequest request) {

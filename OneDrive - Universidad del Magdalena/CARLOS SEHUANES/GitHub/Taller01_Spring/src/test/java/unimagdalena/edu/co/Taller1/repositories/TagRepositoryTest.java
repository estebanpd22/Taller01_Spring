...
class TagRepositoryTest {
    ...
    void shouldFindByNamesWithLimit() {
        ...
        Tag vip = tagRepository.save(Tag.builder().name("VIP").build());
        Tag oferta = tagRepository.save(Tag.builder().name("OFERTA").build());

        List<String> names = List.of("PROMO", "VIP", "OFERTA");

        // WHEN
        List<Tag> encontrados = tagRepository.findByNameIgnoreCaseIn(names, Limit.of(2));

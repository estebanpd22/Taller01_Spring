...
class FlightRepositoryTest {
    ...
    private Flight createFlight(String number, Airline airline, Airport origin,
        ...
        return flightRepository.save(
                Flight.builder()
                        .number(number)
                        .airline(airline)       // 👈 solo la asociación a la aerolínea
                        .origin(origin)
    ...
    void testFindByAirlineName() {
        // given
        Airline airline = Airline.builder().name("Avianca").build();
        airlineRepository.save(airline);

        Airport origin = Airport.builder().code("BOG").build();
        Airport destination = Airport.builder().code("MDE").build();
        ...
        assertThat(flights).allMatch(f ->
                f.getAirline().getName().equalsIgnoreCase("Avianca"));
    }
    ...
    void shouldFindFlightsByRouteAndTimeWindow() {
        ...
        Page<Flight> flights = flightRepository.findByOrigin_CodeAndDestination_CodeAndDepartureTimeBetween(
                "BOG", "MDE", LocalDateTime.now(), pageable
        );
